...
const VOWELS = 'AEIOUY';
const SILENT_START = ['GN', 'KN', 'PN', 'WR', 'PS'] as const;
...
class CodeBuilder {
  ...
  append(primary: string, alternate?: string): void {
...
class Word {
  ...
  contains(start: number, length: number, ...candidates: string[]): boolean {
 ...
 * yields empty codes.
 */
export function doubleMetaphone(
  input: string,
  maxLength: number = DEFAULT_MAX_CODE_LEN,
): DoubleMetaphoneCode {
  const cleaned = input.toUpperCase().replace(/[^A-Z]/g, '');
  if (cleaned === '') return { primary: '', alternate: '' };
...
function skipSilentStart(word: Word): number {
 ...
 */
function step(word: Word, code: CodeBuilder, index: number): number {
...
function stepC(word: Word, code: CodeBuilder, index: number): number {
...
function conditionC0(word: Word, index: number): boolean {
...
function stepCC(word: Word, code: CodeBuilder, index: number): number {
  if (word.contains(index + 2, 1, 'I', 'E', 'H') && !word.contains(index + 2, 2, 'HU')) {
    if (
      (index === 1 && word.at(index - 1) === 'A') ||
...
function stepCH(word: Word, code: CodeBuilder, index: number): number {
...
function conditionCH0(word: Word, index: number): boolean {
  if (index !== 0) return false;
  if (
    !word.contains(index + 1, 5, 'HARAC', 'HARIS') &&
...
function conditionCH1(word: Word, index: number): boolean {
  return (
...
function stepD(word: Word, code: CodeBuilder, index: number): number {
...
function stepG(word: Word, code: CodeBuilder, index: number): number {
  ...
  if (
    (word.contains(index + 1, 2, 'ER') || word.at(index + 1) === 'Y') &&
...
function stepGH(word: Word, code: CodeBuilder, index: number): number {
  ...
  if (
    (index > 1 && word.contains(index - 2, 1, 'B', 'H', 'D')) ||
...
function stepGN(word: Word, code: CodeBuilder, index: number): number {
...
function stepH(word: Word, code: CodeBuilder, index: number): number {
...
function stepJ(word: Word, code: CodeBuilder, index: number): number {
...
function stepL(word: Word, code: CodeBuilder, index: number): number {
...
function conditionL0(word: Word, index: number): boolean {
  if (
    index === word.length - 3 &&
  ...
  return (
    (word.contains(word.length - 2, 2, 'AS', 'OS') || word.contains(word.length - 1, 1, 'A', 'O')) &&
    word.contains(index - 1, 4, 'ALLE')
  );
}

function stepP(word: Word, code: CodeBuilder, index: number): number {
...
function stepR(word: Word, code: CodeBuilder, index: number): number {
  if (
    index === word.length - 1 &&
    !word.isSlavoGermanic() &&
...
function stepS(word: Word, code: CodeBuilder, index: number): number {
  ...
  if (
    (index === 0 && word.contains(index + 1, 1, 'M', 'N', 'L', 'W')) ||
...
function stepSC(word: Word, code: CodeBuilder, index: number): number {
...
function stepT(word: Word, code: CodeBuilder, index: number): number {
...
function stepW(word: Word, code: CodeBuilder, index: number): number {
  ...
  if (
    (index === word.length - 1 && word.isVowel(index - 1)) ||
...
function stepX(word: Word, code: CodeBuilder, index: number): number {
  if (
    !(
      index === word.length - 1 &&
...
function stepZ(word: Word, code: CodeBuilder, index: number): number {
  if (word.at(index + 1) === 'H') {
    code.append('J');
    return index + 2;
  }
  if (
    word.contains(index + 1, 2, 'ZO', 'ZI', 'ZA') ||
...
function isMSilentDoubled(word: Word, index: number): boolean {
  return (
