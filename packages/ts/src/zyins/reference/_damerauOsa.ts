 ...
 */
export function fuzzyThresholdForLength(queryLength: number): number {
  if (queryLength < FUZZY_SHORT_LEN) return 1;
  if (queryLength <= FUZZY_MEDIUM_LEN) return 1;
 ...
 */
export function optimalStringAlignmentDistance(
  a: string,
  b: string,
