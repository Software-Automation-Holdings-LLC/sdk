...
export class DefaultMatchAlgorithm implements MatchAlgorithm {
  ...
  match(query: string, candidates: readonly Concept[]): Concept | UnknownConcept {
    ...
    for (const candidate of candidates) {
      if (_makeKey(candidate.name) === key) return candidate;
      if (candidate.id !== null && _makeKey(candidate.id) === key) return candidate;
    }
    ...
    if (checkKey) {
      for (const candidate of candidates) {
        if (_checkKey(candidate.name) === checkKey) return candidate;
      }
    }
