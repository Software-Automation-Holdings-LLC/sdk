...
export class HttpRequestError extends Error {
    ...
    public readonly isCORS: boolean;
...
export async function httpRequest<T = unknown>(opts: HttpRequestOptions): Promise<T> {
    ...
    if (!url) {
        ...
        if (debug) logger.error('httpRequest error', { error: err, method, body, otherArgs });
        throw err;
    }

    const f = resolveFetchFacade(fetchImpl, HTTP_CONTEXT);
    const timeoutController = new AbortController();
    const timeoutHandle: TimerHandle = timer.setTimeout(() => timeoutController.abort(), timeout);
    ...
    try {
        ...
        if (encoded.wireBody !== undefined) requestInit.body = encoded.wireBody;
        ...
        return (await parseResponse(response, returnType, url, logger, debug)) as T;
    } catch (error) {
 ...
 */
function composeSignals(timeoutSignal: AbortSignal, callerSignal: AbortSignal | null | undefined): { signal: AbortSignal; cleanup: () => void } {
    ...
    if (typeof anyFactory === 'function') return { signal: anyFactory([timeoutSignal, callerSignal]), cleanup: () => undefined };
    ...
    const relay = (src: AbortSignal): void => {
        ...
        const handler = (): void => controller.abort(src.reason);
    ...
    relay(callerSignal);
    const cleanup = (): void => {
        if (timeoutHandler) timeoutSignal.removeEventListener('abort', timeoutHandler);
 ...
 */
function encodeBody(body: unknown): { wireBody: BodyInit; authBody: string; contentType: string | undefined } {
    if (typeof body === 'string') return { wireBody: body, authBody: body, contentType: undefined };
    if (isNativeBodyInit(body)) return { wireBody: body as BodyInit, authBody: '', contentType: undefined };
 ...
 */
function withDefaultContentType(headers: HeadersInit | undefined, value: string): HeadersInit {
    if (!headers) return { 'Content-Type': value };
...
function isNativeBodyInit(value: unknown): boolean {
    ...
    if (typeof g.Blob === 'function' && value instanceof (g.Blob as new () => unknown)) return true;
    if (typeof g.FormData === 'function' && value instanceof (g.FormData as new () => unknown)) return true;
    if (typeof g.URLSearchParams === 'function' && value instanceof (g.URLSearchParams as new () => unknown)) return true;
    if (typeof g.ReadableStream === 'function' && value instanceof (g.ReadableStream as new () => unknown)) return true;
...
async function parseJsonBody(response: Response): Promise<unknown> {
    if (EMPTY_BODY_STATUSES.has(response.status)) return undefined;
...
async function parseResponse(response: Response, returnType: HttpReturnType, url: string, logger: Logger, debug: boolean): Promise<unknown> {
    try {
...
function enhanceError(
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
        ...
        if (ctx.debug) ctx.logger.error(`Abort for ${ctx.url}`, { error: abortErr, timedOut });
        return abortErr;
    }
    ...
    if (error instanceof TypeError) {
        ...
        if (isCORS) {
            ...
            if (error.stack) corsErr.stack = error.stack;
            if (ctx.debug) {
        ...
        if (isNetwork) {
            ...
            if (error.stack) netErr.stack = error.stack;
            if (ctx.debug) ctx.logger.error(`Transient network error for ${ctx.url}`, { error: netErr });
            return netErr;
        }
    }

    if (ctx.debug) ctx.logger.error(`Error fetching ${ctx.url}`, { error, method: ctx.method });
