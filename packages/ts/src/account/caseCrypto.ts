...
export async function encryptCase(
  product: string,
  payload: unknown,
  options: TCaseCryptoOptions = {},
): Promise<TEncryptedCase> {
  ...
  const sealed = new Uint8Array(
    await subtle.encrypt(gcmParams(iv, product), key, toArrayBuffer(plaintext)),
  );
...
function gcmParams(iv: Uint8Array, product: string): AesGcmParams {
  return {
    name: ALGORITHM,
    iv: toArrayBuffer(iv),
...
async function openSealed(
  ...
  try {
    ...
    throw new IsaCaseDecryptError(
      `case envelope failed authentication for product ${product}: wrong key, wrong product, or tampered ciphertext (${(err as Error).message})`,
    );
  }
}
