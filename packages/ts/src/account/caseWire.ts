...
export function parseCreatedId(body: string): string {
...
export function parseCaseDetail(body: string): { product: string; envelope: TCaseEnvelope } {
...
export function parseCaseList(body: string): TCaseListEntry[] {
  if (!body) {
...
function caseRecord(body: string, operation: string): Record<string, unknown> {
  if (!body) {
