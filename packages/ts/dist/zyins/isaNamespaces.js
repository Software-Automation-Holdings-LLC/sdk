...
export class DatasetsFacade {
    ...
    async get(options) {
        ...
        if (isNotModifiedMarker(result)) {
            throw new Error('isa.zyins.datasets.get(): server returned 304 Not Modified for an unconditional fetch — pass no ifNoneMatch, or use getV3() to handle the not-modified case');
        }
        return result;
    }
    /**
