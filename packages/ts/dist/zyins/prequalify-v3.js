...
import { coerceCarrier, coerceMoney, coerceProduct, isRecord, toBool, toNullableNum, toNum, toStr, } from './v3Coercion';
...
export function serializeV3PrequalifyBody(request) {
    ...
    if (isMulti(coverage)) {
        throw new Error('v3 prequalify accepts a single face amount; a multi-amount coverage was supplied. Use Coverage.faceValue(amount) here, or quoteV3 for multi-amount probes.');
    }
    if (coverage.type !== 'face_value') {
        throw new Error(`v3 prequalify accepts only face_value coverage; got ${JSON.stringify(coverage.type)}. Use Coverage.faceValue(amount) here, or quoteV3 for monthly-budget evaluations.`);
    }
...
function coercePrequalifyOffer(raw) {
    const r = isRecord(raw) ? raw : {};
