...
(function (State) {
    ...
    State["Tennessee"] = "TN";
    State["Texas"] = "TX";
    State["UnitedStatesVirginIslands"] = "VI";
    State["Utah"] = "UT";
    State["Vermont"] = "VT";
    State["Virginia"] = "VA";
    State["Washington"] = "WA";
...
const ALL_STATES = Object.freeze(Object.values(State));
export const States = Object.freeze({
    values() {
        return ALL_STATES;
    },
    entries() {
        return ALL_STATES.map((s) => [s, METADATA[s]]);
    },
    metadata(s) {
        const m = METADATA[s];
        if (!m)
            throw new Error(`States.metadata: unknown state '${s}'`);
        return m;
    },
    /**
    ...
    byAbbreviation(abbr) {
        ...
        if (upper in METADATA)
            return upper;
        const fromName = BY_NAME[abbr.toLowerCase()];
        return fromName ? fromName : undefined;
    },
});
