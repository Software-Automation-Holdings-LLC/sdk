 ...
 */
export const stripQuotes = (value) => value.length >= 2 && value.startsWith(QUOTE_CHAR) && value.endsWith(QUOTE_CHAR)
    ? value.slice(1, -1)
    : value;
/**
 ...
 */
export const computeDeviceSignature = async (body, deviceId, subtle) => {
    const cryptoSubtle = resolveSubtle(subtle, LICENSE_CONTEXT);
    const cleanDeviceId = stripQuotes(deviceId);
    const encoder = new TextEncoder();
    const key = await cryptoSubtle.importKey('raw', encoder.encode(cleanDeviceId), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
 ...
 */
export const buildLicenseHeader = (licenseKey, orderId, email) => {
...
function base64Encode(input) {
    try {
        ...
        throw new Error(LICENSE_ERROR_PREFIX + err.message);
    }
}
