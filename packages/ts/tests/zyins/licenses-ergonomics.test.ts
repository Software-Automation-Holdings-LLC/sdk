/**
 * Tightened-ergonomics suite for `isa.zyins.licenses.*`.
 *
 * Covers:
 ...
 *   - Persistence round-trip via mock store + `withLicenseAsync`.
...
describe('isa.zyins.licenses ergonomics', () => {
  ...
  it('activate() stashes the fresh license key into shared AuthContext', async () => {
    ...
    await isa.zyins.licenses.activate();
  ...
  it('activate() with overrides does not mutate instance credentials', async () => {
    ...
    await isa.zyins.licenses.activate({ email: 'override@example.com' });
    expect(isa.credentialState?.auth.licenseKey).toBe('');
  });
  ...
  it('check() defaults from instance but explicit args still win', async () => {
    ...
    await isa.zyins.licenses.check();
    expect(jsonObject(requests[0]!.body)['email']).toBe(TEST_AUTH.email);

    await isa.zyins.licenses.check({ email: 'override@example.com' });
  ...
  it('deactivate() with no args clears the stashed license key', async () => {
    ...
    await isa.zyins.licenses.deactivate();
    expect(isa.credentialState?.auth.licenseKey).toBe('');
  ...
  it('deactivate() with overrides does not clear instance credentials', async () => {
    ...
    await isa.zyins.licenses.deactivate({ email: 'override@example.com' });
  ...
  it('persistence round-trip: stored deviceId + licenseKey are reused', async () => {
    ...
    const { transport, requests } = recording(200, JSON.stringify({ status: 'active' }));
    const isa = await Isa.withLicenseAsync(
      {
        keycode: TEST_AUTH.licenseKey,
        email: TEST_AUTH.email,
        credentialStore: store,
  ...
  it('Isa.fromEnv selects license mode when ISA_LICENSE_* is set', () => {
    const isa = Isa.fromEnv(licenseEnv());
    expect(isa.identity.mode).toBe('license');
  });

  it('Isa.fromEnv selects bearer mode when ISA_TOKEN is set', () => {
    const env: EnvReader = {
      get: (n) => (n === ENV_VAR_NAMES.bearer.token ? 'isa_test_token' : undefined),
    };
    const isa = Isa.fromEnv(env);
    expect(isa.identity.mode).toBe('bearer');
  });

  it('Isa.fromEnv skips partial license env and selects complete session env', () => {
    const env: EnvReader = {
      get: (n) => {
        if (n === ENV_VAR_NAMES.license.keycode) return TEST_AUTH.licenseKey;
  ...
  it('mints a deviceId when none is supplied', () => {
    const isa = Isa.withLicense(
